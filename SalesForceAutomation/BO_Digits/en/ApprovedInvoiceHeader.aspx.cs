...
{
	public partial class ApprovedInvoiceHeader : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			if (!Page.IsPostBack)
			{
				...
				try
				{
					if (Session["AIHFromDate"] != null)
					{
						rdfromDate.SelectedDate = DateTime.Parse(Session["AIHFromDate"].ToString());
					}
					else
					{
						rdfromDate.SelectedDate = DateTime.Parse(DateTime.Now.ToString("MMM-yyyy"));
					}
					if (Session["AIHToDate"] != null)
					{
						rdendDate.SelectedDate = DateTime.Parse(Session["AIHToDate"].ToString());
					}
					else
					{
				...
				if (Session["AIHrotID"] != null)
					{
						int a = rdRoute.Items.Count;
						string rotID = Session["AIHrotID"].ToString();
					...
					if (Session["AIHcusID"] != null)
					{
						int a = rdCustomer.Items.Count;
						string cusID = Session["AIHcusID"].ToString();
					...
					catch (Exception ex)
					{
						...
						ObjclsFrms.LogMessageToFile(UICommon.GetLogFileName(), "ApprovedInvoiceHeader.aspx PageLoad()", "Error : " + ex.Message.ToString() + " - " + innerMessage);
					}
		...
		public void ListData()
		{
			...
			if (!rotID.Equals("0"))
			{
				...
				DataTable lstdata = ObjclsFrms.loadList("ApprovedInvoiceHeader", "sp_ServiceRequest", mainCondition);
		...
		protected void grvRpt_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
		{
				...
				if (e.CommandName.Equals("Detail"))
			{
				...
				Response.Redirect("ApprovedInvoiceDetail.aspx?ID=" + ID);
			}
		}
		...
		protected void lnkFilter_Click(object sender, EventArgs e)
		{
				try
				{
					if (Session["AIHFromDate"] != null)
					{
						...
						if (fromdate == Session["AIHFromDate"].ToString())
						{
							rdfromDate.SelectedDate = DateTime.Parse(Session["AIHFromDate"].ToString());
						}
						else
						{
							Session["AIHFromDate"] = DateTime.Parse(rdfromDate.SelectedDate.ToString());
						}
					}
					else
					{
					...
					if (Session["AIHToDate"] != null)
					{
						...
						if (todate == Session["AIHToDate"].ToString())
						{
							rdendDate.SelectedDate = DateTime.Parse(Session["AIHToDate"].ToString());
						}
						else
						{
							Session["AIHToDate"] = DateTime.Parse(rdendDate.SelectedDate.ToString());
						}
					...
					{
						string route = Rot();
						if (route == Session["AIHrotID"].ToString())
						{
							string rotID = Rot();
					...
					{
						string customer = Cus();
						if (customer == Session["AIHcusID"].ToString())
						{
							string cusID = Cus();
