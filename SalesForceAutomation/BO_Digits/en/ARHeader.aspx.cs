    ...
    public partial class ARHeader : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)            
            {
                ...
                try
                {
                    if (Session["ARAdvFilter"] != null)
                    {
                        if (Session["ARAdvFilter"].ToString() == "true")
                        {
                            plhFilter.Visible = true;
                        }
                        else
                ...
                else
                {
                    try
                    {
                        ...
                        if (Session["ARrotID"] != null)
                        {
                            int a = rdRoute.Items.Count;
                            string route = Session["ARrotID"].ToString();
                            string[] ar = route.Split(',');
                            for (int i = 0; i < ar.Length; i++)
                            {
                                foreach (RadComboBoxItem items in rdRoute.Items)
                    ...
                    try
                    {
                        if (Session["ARcusID"] != null)
                        {
                            int a = rdCustomer.Items.Count;
                            string cusID = Session["ARcusID"].ToString();
        ...
        protected void lnkDOwnload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string route = Rot();
                    if (route == Session["ARrotID"].ToString())
                    {
                        string rotID = Rot();
                ...
                {
                    string customer = Cus();
                    if (customer == Session["ARcusID"].ToString())
                    {
                        string cusID = Cus();
        ...
        protected void grvRpt_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("Detail"))
            {
                ...
                Response.Redirect("ARDetail.aspx?Id=" + ID + "&&mode=" + PayType);
            }
        }
        ...
        protected void grvRpt_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                ...
                string imah = dataItem["arh_RecieptImg"].Text.Replace(" ", "");
                ...
                string imagC = dataItem["arp_Image1"].Text.Replace(" ", "");
                ...
                HyperLink arh_Attachment = (HyperLink)item.FindControl("arh_Attachmen");
                string SignedCopy = arh_Attachment.ImageUrl.ToString();
                if (SignedCopy.Equals("../"))
                {
                    arh_Attachment.ImageUrl = "";
                    arh_Attachment.Text = "<span style='font-size: 11px; color: black; white-space: nowrap;'>No Attachment</span>";

                }
                else
                {
                    arh_Attachment.ImageUrl = "../assets/media/svg/Files/File.svg";
                }
        ...
        protected void lnkAdvFilter_Click(object sender, EventArgs e)
        {
            ...
            {
                plhFilter.Visible = false;
                Session["ARAdvFilter"] = "false";
            }
            else
            {
                plhFilter.Visible = true;
                Session["ARAdvFilter"] = "true";
            }
        }
