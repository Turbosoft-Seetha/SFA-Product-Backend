...
{
    public partial class PromotionalItems : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                try
                {
                    if (Session["PRIFDate"] != null)
                    {

                        rdfromDate.SelectedDate = DateTime.Parse(Session["PRIFDate"].ToString());
                    }
                    else
                    {
                    ...
                    if (Session["PRITDate"] != null)
                    {

                        rdendDate.SelectedDate = DateTime.Parse(Session["PRITDate"].ToString());
                    }
                    else
                    {
                ...
                try
                {
                    if (Session["PRIrotID"] != null)
                    {
                        int a = rdRoute.Items.Count;
                        string route = Session["PRIrotID"].ToString();
        ...
        public void LoadData()
        {
            ...
            if (!rotID.Equals("0"))
            {
                ...
                lstDatas = ObjclsFrms.loadList("ListPromotionalItems", "sp_Report", mainCondition);
        ...
        protected void grvRpt_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            if (e.CommandName.Equals("Invoice"))
            {
                try
                {
                    ...
                    string salNo = dataItem["sal_ID"].Text.ToString();
                   
                    string url = ConfigurationManager.AppSettings.Get("BackendURL");
                    string s = "BO_Digits/en/SalesDetails.aspx?Id="+ salNo+ "&&Type=";
                    url = url + s;
                ...
                catch (Exception ex)
                {
                    ...
                    ObjclsFrms.LogMessageToFile(UICommon.GetLogFileName(), "PromotionalItems.aspx", "Error : " + ex.Message.ToString() + " - " + innerMessage);
                }
        ...
        protected void Filter_Click(object sender, EventArgs e)
        {
            try
            {
                if (Session["PRIFDate"] != null)
                {
                    ...
                    if (fromdate == Session["PRIFDate"].ToString())
                    {
                        rdfromDate.SelectedDate = DateTime.Parse(Session["PRIFDate"].ToString());
                    }
                    else
                    {
                        Session["PRIFDate"] = DateTime.Parse(rdfromDate.SelectedDate.ToString());
                    }
                }
                else
                {
                ...
                if (Session["PRITDate"] != null)
                {
                    ...
                    if (todate == Session["PRITDate"].ToString())
                    {
                        rdendDate.SelectedDate = DateTime.Parse(Session["PRITDate"].ToString());
                    }
                    else
                    {
                        Session["PRITDate"] = DateTime.Parse(rdendDate.SelectedDate.ToString());
                    }
                ...
                {
                    string route = Rot();
                    if (route == Session["PRIrotID"].ToString())
                    {
                        string rotID = Rot();
        ...
        private void SpreadStreamProcessingForXLSXAndCSV(DataTable dt, SpreadDocumentFormat docFormat = SpreadDocumentFormat.Xlsx, string sheetName = "Sheet1")
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                Response.AppendHeader("Content-Disposition", string.Format("attachment; filename={0}.{1}", "Promotional Items", "Xlsx"));
